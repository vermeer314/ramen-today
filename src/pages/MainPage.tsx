...
export const MainPage = () => {
  ...
  const todayDragProps = useDragScroll();
  const closingDragProps = useDragScroll();
  const upcomingDragProps = useDragScroll();
            ...
            {todayEvents.length > 0 ? (
              todayEvents.map((item, index) => (
                <StoryBadge
                  ...
                  item={item}
                  isEvent={true}
                  onClick={() => openStory(todayEvents, index)}
                />
              ))
            ) : (
              <EmptyState
                message="아직 들려온 이벤트 소식이 없어요."
                subMessage="혹시 알고 계신 게 있나요? 👀"
              />
            )}
          </div>
        </section>
            ...
            {closings.length > 0 ? (
              closings.map((item, index) => (
                <StoryBadge
                  ...
                  item={item}
                  isEvent={false}
                  onClick={() => openStory(closings, index)}
                />
              ))
            ) : (
              <EmptyState
                message="다행히 갑작스러운 휴무 소식은 없어요!"
                subMessage="평화로운 라멘 투데이 ✌️"
              />
            )}
          </div>
        </section>
        ...
        <section className="shrink-0">
          ...
          >
            {upcomingEvents.length > 0 ? (
                ...
                <div className="md:hidden flex gap-3">
                  {upcomingEvents.map((item, index) => (
                    <StoryBadge
                      ...
                      item={item}
                      isEvent={true}
                      onClick={() => openStory(upcomingEvents, index)}
                    />
                  ))}
                </div>
              ...
              <EmptyState
                message="예정된 이벤트 소식이 없어요."
                subMessage="새로운 이벤트 일정을 기대해 주세요! 🍜"
              />
            )}
          </div>
        </section>
...
const EmptyState = ({
  ...
  <div className="w-full min-w-[280px] h-[104px] md:h-[120px] flex items-center justify-center p-5 bg-white rounded-2xl text-[12px] md:text-[13px] font-medium text-gray-500 text-center border border-gray-100 shadow-sm">
    {message} <br className="hidden md:block" /> {subMessage}
  </div>
);
